...
impl BuzHash2 {
    ...
    pub fn push(&mut self, inc: u8) {
        let out = self.buf[self.index];
        self.hash_sum = self.hash_sum.rotate_left(1)
            ^ BUZHASH_TABLE[out as usize].rotate_left(self.window as u32)
            ^ BUZHASH_TABLE[inc as usize];
        self.buf[self.index] = inc;
...
impl RollingHasher for BuzHash2 {
    fn reset(&mut self) {
        self.reset();
    }
